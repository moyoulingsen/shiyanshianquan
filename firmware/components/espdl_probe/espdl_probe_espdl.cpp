...
static void run_probe_model(dl::Model *model, int64_t load_start_us)
{
    ...
    if (model->get_fbs_model() == nullptr) {
        ESP_LOGE(TAG,
                 "dl::Model construction failed after %lld us: internal FbsModel is empty",
                 (long long)(esp_timer_get_time() - load_start_us));
        delete model;
        return;
    }
    ...
    if (test_ret == ESP_OK) {
        ...
        ESP_LOGE(TAG,
                 "model->test() failed: %s, elapsed=%lld us",
    ...
    ESP_LOGI(TAG,
             "model->profile_module(sort_by_latency=%d)",
...
{
#if CONFIG_LABGUARD_ESPDL_PROBE_LOAD_SDCARD
    ...
    if (stat(model_path, &model_stat) != 0) {
        ESP_LOGE(TAG, "model file is not accessible: %s", model_path);
        return;
    }
    if (model_stat.st_size <= 0) {
        ESP_LOGE(TAG, "model file is empty: %s", model_path);
        return;
    }

    ESP_LOGI(TAG, "loading .espdl model from SD card path: %s", model_path);
    const int64_t load_start_us = esp_timer_get_time();
    dl::Model *model = new (std::nothrow) dl::Model(model_path, fbs::MODEL_LOCATION_IN_SDCARD);
    run_probe_model(model, load_start_us);
#elif CONFIG_LABGUARD_ESPDL_PROBE_LOAD_RODATA
